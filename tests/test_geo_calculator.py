...
class TestCameraPitchConvention:
    ...
    def test_minus_60_center_projects_forward_on_ground(self):
        calc = GeoCalculator()
        ...
        lat, lon = calc.pixel_to_gps((960, 540), telemetry)

        expected_north_m = 75 / math.tan(math.radians(60))
        actual_north_m = calc.calculate_distance_m(
            telemetry.latitude,
            telemetry.longitude,
            lat,
            telemetry.longitude,
        )
        assert lat > telemetry.latitude
        assert lon == pytest.approx(telemetry.longitude)
        assert actual_north_m == pytest.approx(expected_north_m, abs=0.1)
...
class TestGeoCalculator:
    ...
    def test_pixel_to_gps_frame_center(self, calculator, base_telemetry):
        ...
        expected_north_m = 75 / math.tan(math.radians(70))
        actual_north_m = calculator.calculate_distance_m(
            base_telemetry.latitude,
            base_telemetry.longitude,
            lat,
            base_telemetry.longitude,
        )
        assert lat > base_telemetry.latitude
        assert lon == pytest.approx(base_telemetry.longitude)
        assert actual_north_m == pytest.approx(expected_north_m, abs=0.1)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("drone_yaw_deg", float("nan")),
            ("camera_yaw_deg", float("inf")),
            ("hfov_deg", float("nan")),
        ],
    )
    def test_rejects_non_finite_telemetry(
        self,
        calculator,
        field,
        value,
    ):
        telemetry = make_telemetry(**{field: value})

        with pytest.raises(ValueError, match="finite"):
            calculator.pixel_to_gps((960, 540), telemetry)
